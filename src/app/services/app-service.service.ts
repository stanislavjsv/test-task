...
export class AppService {
  ...
  getArrAll(arr: Array<object>){
    for(let i = 0; i < this.data.data.length; i++){
      arr.push(this.data.data[i])
    }
  }

  getArrIncome(arr: Array<object>){
    for(let i = 0; i < this.data.data.length; i++){
      if(this.data.data[i].type === 'income'){
        arr.push(this.data.data[i])
      }
    }
  }

  getArrInvestment(arr: Array<object>){
    for(let i = 0; i < this.data.data.length; i++){
      if(this.data.data[i].type === 'investment'){
        arr.push(this.data.data[i])
      }
    }
  }

  getArrOutcome(arr: Array<object>){
    for(let i = 0; i < this.data.data.length; i++){
      if(this.data.data[i].type === 'outcome'){
        arr.push(this.data.data[i])
      }
    }
  }

  getArrLoan(arr: Array<object>){
    for(let i = 0; i < this.data.data.length; i++){
      if(this.data.data[i].type === 'loan'){
        arr.push(this.data.data[i])
      }
    }
  }
